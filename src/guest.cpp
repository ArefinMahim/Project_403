...
void Guest::setCheckInDate(const string CheckInDate) { checkInDate = CheckInDate; }
void Guest::setCheckOutDate(const string CheckOutDate) { checkOutDate = CheckOutDate; }
...
const vector<BookingRecord>& Guest::getBookingHistory() const{ return bookingHistory; }
...
bool Guest::verifyPassword(const string& pw) const{ return passwordHash==to_string(hashPassword(pw));}
void Guest::changePassword(const string& newPw){ passwordHash=to_string(hashPassword(newPw));}

void Guest::addBookingRecord(const BookingRecord& b){
    bookingHistory.push_back(b);
    int total=(int)bookingHistory.size();
    if(total>=10)       tier=MemberTier::PLATINUM;
    else if(total>=5)   tier=MemberTier::GOLD;
    else if(total>=2)   tier=MemberTier::SILVER;
    else                tier=MemberTier::BRONZE;
}

void Guest::addBookingRaw(const BookingRecord& b){bookingHistory.push_back(b);}

// Display Guest Information
void Guest::DisplayInfo() {
    Person::DisplayInfo();
    cout << "NID: " << getNid() << endl;
    if(roomNo>0)                cout<<"Room No: "<<getRoomNo()<<endl;
    if(!checkInDate.empty())    cout<<"Check-In-Date: "<<getCheckInDate()<<endl;
    if(!checkOutDate.empty())   cout<<"Chech-Out-Date: "<<getCheckOutDate()<<endl;
